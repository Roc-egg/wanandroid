...
import 'package:wanandroid/config/storage_manager.dart';
import 'package:wanandroid/generated/l10n.dart';
import 'package:wanandroid/ui/helper/theme_helper.dart';
...
class ThemeModel with ChangeNotifier {
  ...
  /// 当前主题颜色
  MaterialColor _themeColor = Colors.primaries[
      StorageManager.sharedPreferences.getInt(kThemeColorIndex) ?? 5];

  /// 当前字体索引
  int _fontIndex = StorageManager.sharedPreferences.getInt(kFontIndex) ?? 0;

  ///获取_fontIndex字体索引
  int get fontIndex => _fontIndex;

  /// 切换指定色彩
  /// 没有传[userDarkMode]就不改变brightness,color同理
...
//          textTheme: CupertinoTextThemeData(brightness: Brightness.light)
  ...
  void saveTheme2Storage(bool userDarkMode, MaterialColor themeColor) async {
    var indexOf = Colors.primaries.indexOf(themeColor);
    await Future.wait([
      StorageManager.sharedPreferences
          .setBool(kThemeUserDarkMode, userDarkMode),
      StorageManager.sharedPreferences.setInt(kThemeColorIndex, indexOf)
    ]);
  }
  ...
  /// 字体选择持久化
  void saveFontIndex(int index) async {
